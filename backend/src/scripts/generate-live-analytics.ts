        ...
        for (const enrollment of enrollments) {
            ...
            exams.forEach(exam => {
                ...
                marksForStudent.forEach(mark => {
                    ...
                    if (coId) {
                        const current = studentCoMarks.get(coId) || { obtained: 0, max: 0 };
                        studentCoMarks.set(coId, {
                            obtained: current.obtained + (Number(mark.marksObtained || mark.marks) || 0),
                            max: current.max + (mark.subQuestion?.maxMarks || 0)
                        });
                    }
                });
            });
