...
export function ConversationList({ conversations, activeConversation, onSelect }: Props) {
  const { user } = useAuth();
  const { onlineUsers, typingUsers, setActiveConversation, refreshConversations, createConversation } = useMessaging();
  ...
  useEffect(() => {
    ...
    const delay = setTimeout(async () => {
      try {
        setIsSearching(true);
        const { data } = await api.get(`/messaging/search?q=${searchQuery}`);
  ...
  const handleStartContactChat = async (contactId: string) => {
    try {
      ...
      <div className="p-3 bg-muted/20 border-b border-border/50">
        <div className="relative">
          ...
          <Input
            ...
            className="pl-9 h-9 bg-background border-border shadow-none focus-visible:ring-1"
          />
          {searchQuery && (
            <button 
            ...
            >
              <Users className="w-3 h-3 rotate-45" />
            </button>
          )}
        </div>
      </div>
        ...
        {searchQuery && searchResults ? (
          <div className="bg-background animate-in fade-in duration-200">
            ...
            {searchResults.conversations.length > 0 && (
              <div className="py-2">
                <div className="px-4 py-1 text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Chats</div>
                {searchResults.conversations.map(conv => (
                  <button
                    key={conv.id}
                    onClick={() => { onSelect(conv); setSearchQuery(''); }}
                    className="w-full px-4 py-2 text-left hover:bg-muted/50 flex items-center gap-3"
                  >
                    <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-xs font-bold text-primary">
                      {conv.type === 'GROUP' ? <Users className="w-4 h-4" /> : (conv.name?.charAt(0) || 'C')}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate">{conv.name || conv.participants.find((p: any) => p.user.id !== user?.id)?.user.fullName || 'Group'}</p>
                    </div>
                  </button>
                ))}
              </div>
            ...
            {searchResults.messages.length > 0 && (
              <div className="py-2 border-t border-border/50">
                <div className="px-4 py-1 text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Messages</div>
                {searchResults.messages.map(msg => (
                  <button
                    key={msg.id}
                    onClick={() => { onSelect(msg.conversation); setSearchQuery(''); }}
                  ...
                  >
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className="text-[10px] text-muted-foreground italic">in {msg.conversation.name || 'Private Chat'}</span>
                    </div>
            ...
            {searchResults.contacts.length > 0 && (
              <div className="py-2 border-t border-border/50">
                <div className="px-4 py-1 text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Recommendations</div>
                {searchResults.contacts.map(contact => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-muted-foreground truncate">{contact.role} • {contact.registrationNumber || contact.email}</p>
                    </div>
                  </button>
                ))}
              </div>
            ...
            {searchResults.conversations.length === 0 && searchResults.messages.length === 0 && searchResults.contacts.length === 0 && (
